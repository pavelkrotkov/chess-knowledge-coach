...
def test_streaming_puzzle_import_is_versioned_and_queryable(tmp_path) -> None:
    ...
    source.write_text(
        "PuzzleId,FEN,Moves,Rating,RatingDeviation,Themes,OpeningTags\n"
        "p1,fen1,e2e4 e7e5,1200,80,fork middlegame,Italian_Game\n"
        "p2,fen2,d2d4 d7d5,1800,60,backRank endgame,Queen's_Gambit\n",
        encoding="utf-8",
    )
    db = Database(":memory:")
    db.initialize()
    ...
    assert tuple(stored) == ("e2e4 e7e5", "e2e4 e7e5")
